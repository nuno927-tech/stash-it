/// A mark for each kind of document.
///
/// The paths are copied verbatim from `GLYPH` in `src/components/PaperIcon.tsx`
/// — line drawings rather than the emoji-adjacent glyphs a document list
/// usually gets, and the same argument as the nav icons: Material has a
/// passport-shaped rectangle and this app has a booklet with a crest.
///
/// ── The stroke is the whole trick ─────────────────────────────────────────
/// At 20px inside a 36px tile these have to survive the test the nav icons
/// did: the pen has to be thin enough relative to the picture that the shape
/// reads. That is why it is 1.7 and not 2, and why drawing one bigger buys no
/// detail at all — a stroke scales with its viewBox.
///
/// ── The colour is the state, and only the state ───────────────────────────
/// Nothing here is decorative, so a green row means green news. The same three
/// states as everywhere else map onto moss, honey and ember.
library;
...
const Map<PaperKind, String> _glyphs = {
...
/// One kind's mark, in one colour.
class PaperGlyph extends StatelessWidget {
  const PaperGlyph(this.kind, {required this.color, this.size = 20, super.key});
...
/// The mark on its own tinted disc, which is how it appears everywhere.
class PaperMark extends StatelessWidget {
  const PaperMark(
    this.kind, {
    this.state = PaperState.valid,
    this.size = 36,
    super.key,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final (ink, wash) = switch (state) {
      PaperState.valid => (c.moss, c.washMoss),
      PaperState.renew => (c.honey, c.washHoney),
      PaperState.expired => (c.ember, c.washEmber),
    };
    ...
    return Container(
      ...
      child: PaperGlyph(kind, color: ink, size: size * 0.56),
    );
  }
}
